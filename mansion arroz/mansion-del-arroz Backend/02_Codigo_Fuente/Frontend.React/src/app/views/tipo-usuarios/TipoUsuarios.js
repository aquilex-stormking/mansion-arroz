...
import ModalTipoUsuarios from "../../components/tipo-usuarios/ModalTipoUsuarios";
...
export default function TipoUsuarios() {
  ...
  const getTipoUsuario = (data) => {
    TipoUsuariosService.getTipoUsuario(data).then((response) => {
      if (response?.result) {
  ...
  const deleteItem = (item) => {
    TipoUsuariosService.deleteTipoUsuario({ userTypeId: item.userTypeId }).then(
      (response) => {
        if (response?.result) {
          getTipoUsuario({});
        }
  ...
  return (
    <Row>
      ...
      <ModalTipoUsuarios
        ...
        getConsultaData={getTipoUsuario}
